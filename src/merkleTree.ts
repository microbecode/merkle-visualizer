...
function hashValue(value: string, fn: HashFunction): string {
  ...
  if (fn === "ripemd160")
    return new RIPEMD160().update(Buffer.from(value)).digest("hex");
  return value;
}

function hashParentNode(
  leftHash: string,
  rightHash: string,
  fn: HashFunction,
  combineMethod: "concat" | "sum" = "concat",
  commutative = false
): string {
  let combined: string;
...
export function getMerkleProof(
  ...
  commutative = false,
  combineMethod: "concat" | "sum" = "concat"
): { proof: string[]; leaf: string; leafIndex: number } | null {
  ...
  const paddedLeaves =
    padCount > 0
      ? leaves.concat(Array(padCount).fill(leaves[leaves.length - 1]))
      : leaves;
  // Hash all leaves
  ...
  while (level.length > 1) {
    ...
    for (let i = 0; i < level.length; i += 2) {
      ...
      nextLevel.push(
        hashParentNode(left, right, fn, combineMethod, commutative)
      );
    }
